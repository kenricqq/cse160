...
// Fur colors
const WHITE_FUR = [0.95, 0.93, 0.86, 1], CREAM_FUR = [1, 0.97, 0.86, 1], BLACK_FUR = [0.015, 0.014, 0.018, 1];
// Detail colors
const NOSE = [0, 0, 0, 1], PUPIL = NOSE, CLAW_COLOR = [0.86, 0.82, 0.68, 1], EYE_SHINE = [1, 1, 0.95, 1];
const PANDA_BASE_Y = -0.22, TAU = Math.PI * 2, WALK_RATE = 0.36;
const STEP_PHASE = { rearLeft: 0, frontLeft: 0.23, rearRight: 0.5, frontRight: 0.73 };
const NO_MOTION = { bob: 0, roll: 0, headBob: 0, headTilt: 0 };
// Body blocks
const BODY_BOXES = [
...
const idleHeadTilt = (seconds) => 1.2 * Math.sin(TAU * (seconds * WALK_RATE * 2 + 0.18));
// App state
class App {
    constructor(canvas, gl) {
        ...
        this.baseRotationY = -41;
        this.dragRotationX = -0.5;
        this.dragRotationY = 0;
        this.elapsedSeconds = 0;
        this.startTime = performance.now() / 1000;
        this.walking = false;
        this.poking = false;
        this.pokeStartSeconds = 0;
        this.isDragging = false;
        this.lastPointerX = 0;
        this.lastPointerY = 0;
        this.sliderPose = zeroPose();
        this.currentPose = zeroPose();
        this.tick = (now) => {
...
function renderScene(app) {
    ...
    app.renderer.setGlobalRotation(new Matrix4()
...
// Panda root
function drawPanda(r, seconds, pose, walking) {
    const cycle = seconds * WALK_RATE, gait = walking ? walkMotion(cycle) : NO_MOTION;
...
// Head group
function drawHead(r, root, gait, pose) {
...
// Leg groups
function drawLegs(r, root, cycle, pose, walking) {
    const legData = [
        [
            -0.39,
            -0.26,
    ...
    for (const [x, z, angles, frontLeg] of legData)
        drawLeg(r, root, x, z, angles, frontLeg);
}
// Ear sphere
function drawEar(r, head, side) {
    r.sphere(head, BLACK_FUR, [0.005, 0.255, 0.225 * side], [0.2, 0.2, 0.2], 8 * side);
}
// Eye details
function drawEye(r, head, side) {
    const z = 0.118 * side;
    r.boxRot(head, BLACK_FUR, [-0.252, 0.075, z], [0.032, 0.155, 0.12], [-12 * side, 0, 0]);
    r.boxRot(head, BLACK_FUR, [-0.259, 0.005, z + 0.018 * side], [0.024, 0.075, 0.09], [-12 * side, 0, 0]);
    r.box(head, PUPIL, [-0.278, 0.088, z], [0.014, 0.034, 0.04]);
    r.box(head, EYE_SHINE, [-0.288, 0.1, z - 0.014 * side], [0.008, 0.011, 0.012]);
}
// Joint chain
function drawLeg(r, root, x, z, angles, frontLeg) {
...
function walkMotion(cycle) {
    ...
    const lifts = phases.map(lift), swing = lifts[2] + lifts[3] - lifts[0] - lifts[1], foreLift = lifts[1] + lifts[3];
    return {
...
// Limb angles
function limbPose(cycle, start, side, frontLeg) {
    const p = phase(cycle, start), l = lift(p), shoulder = (frontLeg ? 267 : 286) + (frontLeg ? 31 : 34) * Math.cos(p) + (frontLeg ? 1.2 : -1.2) * side, elbow = (frontLeg ? 13 : -7) + (frontLeg ? -10 : -17) * Math.cos(p) + (frontLeg ? 18 : -16) * l;
    return [shoulder, elbow, (frontLeg ? 188 : 190) - shoulder - elbow + (frontLeg ? 7 : -5) * l];
}
function drawBoxes(r, base, boxes) {
    for (const [color, translate, scale, rotateZ = 0] of boxes)
        r.box(base, color, translate, scale, rotateZ);
}
function phase(cycle, start) {
    return TAU * (cycle - start);
}
function lift(p) {
    return Math.max(0, Math.sin(p)) ** 2;
}
function support(phases) {
