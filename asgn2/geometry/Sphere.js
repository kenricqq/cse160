...
class Sphere extends Geometry {
    constructor(rows = 8, cols = 16) {
        ...
        const pushVertex = (theta, phi) => {
            ...
            vertices.push(radiusAtTheta * Math.cos(phi), 0.5 * Math.cos(theta), radiusAtTheta * Math.sin(phi), 1, 1, 1);
        };
        // Latitude bands
        for (let row = 0; row < rows; row++) {
            const theta0 = (row / rows) * Math.PI, theta1 = ((row + 1) / rows) * Math.PI;
            for (let col = 0; col < cols; col++) {
                const phi0 = (col / cols) * Math.PI * 2, phi1 = ((col + 1) / cols) * Math.PI * 2;
