...
class ObjModel extends Geometry {
    vertices;
    constructor(objSource, color = [0.85, 0.7, 0.48]) {
        super();
        this.positionSize = 3;
        this.colorSize = 3;
    ...
    parseObj(objSource, color) {
        ...
        function normalFromTriangle(a, b, c) {
        ...
        function addVertex(vertex, fallbackNormal) {
            let normal = vertex.normal ?? fallbackNormal;
            data.push(...vertex.position, ...color, vertex.texCoord[0] ?? 0, vertex.texCoord[1] ?? 0, ...normal);
        }
        for (let rawLine of objSource.split('\n')) {
            let line = rawLine.trim();
            if (!line || line.startsWith('#'))
